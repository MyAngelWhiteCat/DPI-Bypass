...
void DPIBypasser::AddBypassIngnoreHostname(const std::string_view hostname) {
    resource_to_bypass_method_[std::string(hostname)] = BypassMethod::NON;
}
...
void DPIBypasser::IncrementIdSeqNumAndSetLen(char* packet,
    ...
    PWINDIVERT_TCPHDR tcphdr = reinterpret_cast<PWINDIVERT_TCPHDR>
        (packet + 20);
    SetLength(iphdr, len);
...
void DPIBypasser::DeadFake() {
    RaiiPacket fake_client_hello = GetFakeSni();
    PWINDIVERT_IPHDR fake_iphdr = reinterpret_cast<PWINDIVERT_IPHDR>(fake_client_hello.data());
    PWINDIVERT_TCPHDR fake_tcphdr = reinterpret_cast<PWINDIVERT_TCPHDR>
        (fake_client_hello.data() + fake_iphdr->HdrLength * WORD_WEIGHT);
    fake_iphdr->TTL = 6;
    IncrementTimeStamp(fake_tcphdr, -600000);
    for (int i = 0; i < 6; ++i) {
        SendPacket(fake_client_hello.data(), data_offset_ + fake_iphdr->Length, true);
    }
...
void DPIBypasser::SendMaskedPacket(
    ...
    PWINDIVERT_TCPHDR mask_tcphdr = reinterpret_cast<PWINDIVERT_TCPHDR>
        (mask + mask_iphdr->HdrLength * WORD_WEIGHT);
...
RaiiPacket DPIBypasser::GetCurrentCapturedPacketHeaders() {
    RaiiPacket headers;
    memcpy(headers.data(), packet_.data(), data_offset_);
    return headers;
}
...
bool DPIBypasser::RecvPacket() {
    try {
        ...
        if (packet_len_ < CLIENT_HELLO_PACKET_MINIMUM_SIZE) {
...
void DPIBypasser::IncrementTimeStamp(PWINDIVERT_TCPHDR tcphdr, INT ts_increment) {
    ...
    while (pos + 1 < tcphdr_len) {
        ...
        if (tcphdr_bytes[pos] == ts_type && tcphdr_bytes[pos + 1] == ts_len) {
            ...
            *ts_value_ptr = htonl(ts_value + ts_increment);
            return;
        }
...
RaiiPacket DPIBypasser::GetFakeSni() {
    RaiiPacket fake_client_hello = GetCurrentCapturedPacketHeaders();
    ...
    fake_tls.read(fake_client_hello.data() + data_offset_, fake_packet_len);

    reinterpret_cast<PWINDIVERT_IPHDR>(fake_client_hello.data())
        ->Length = htons(data_offset_ + fake_packet_len);

    return fake_client_hello;
}
...
void DPIBypasser::SendFakeSni(int repeats) {
    ...
    PWINDIVERT_IPHDR fake_iphdr = reinterpret_cast<PWINDIVERT_IPHDR>(fake_client_hello.data());
    PWINDIVERT_TCPHDR fake_tcphdr = reinterpret_cast<PWINDIVERT_TCPHDR>
        (fake_client_hello.data() + 20);
   
    IncrementTimeStamp(fake_tcphdr, -600000);
    for (int i = 0; i < repeats; ++i) {
        SendPacket(fake_client_hello.data(), data_offset_ + fake_iphdr->Length, true, false);
    }
}
